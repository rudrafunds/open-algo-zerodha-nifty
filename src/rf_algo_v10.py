...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        ...
        if instrument_token == global_config['system']['nifty_token']:
            ...
            # Wait for Open Order
            # Retrieve open orders from Kite
            orders = global_kite.orders()
            open_orders = [
            ...
            if open_orders:
                ...
                for order in open_orders:
                    message += f"Position: {order['transaction_type']} @ {order['price']} ({current_price}, {order['status']})"
                    ...
                    if not global_config['system']['only_progress_status']:   
                        ...
                        if global_config['system']['termux']:
                            if direction in ('▲T', '▼T'):
                                ...
                                bar = f'{format_value(current_price, entry_price)}|{filled}{empty}|{target_price:.0f}'
                            else:
                                filled = '🟥' * filled_length
                                bar = f'{format_value(current_price, entry_price)}|{empty}{filled}|{entry_price:.0f}'
...
def position_open(signal_type, net_mtm):
    ...
    try:
        ...
        # Handle buy depth
        buy_depth = global_config['rules']['risk']['order_buy_depth']
        buy_price = instrument['ltp'] - buy_depth
        logging.info(f"\nPosition: Buying: {signal_type} - {instrument['tradingsymbol']} @ {buy_price} (LTP-{instrument['ltp']})...")
        ...
        global_current_position = {
            ...
            'sl': buy_price - (global_config['rules']['risk']['stop_loss']),
            'sl_25': 0,
            'target': buy_price + (global_config['rules']['risk']['target']),
            'tsl': 0,
            'tsl_25': 0,
            'tsl_50': 0,
            'tsl_75': 0,
        ...
        # send telegram notification
        message = f"{CONST_SYMBOL_DICT['Buy']} {CONST_SYMBOL_DICT[signal_type]} Order Buy {instrument['tradingsymbol']} {lot_size}Qty @ {buy_price}(LTP-{instrument['ltp']})"
