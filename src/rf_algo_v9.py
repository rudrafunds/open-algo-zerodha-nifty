...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        ...
        if global_current_position and instrument_token == global_current_position['instrument']['instrument_token']:
            running_mtm = global_current_position['quantity'] * (current_price - global_current_position['entry_price'])
            ...
            if not global_config['system']['only_progress_status']:   
                ...
                if global_config['system']['termux']:
                    if direction in ('▲T', '▼T'):
                        ...
                        bar = f'{format_value(current_price, entry_price)}|{filled}{empty}|{target_price:.2f}'
                    else:
                        filled = '🟥' * filled_length
                        bar = f'{format_value(current_price, entry_price)}|{empty}{filled}|{entry_price:.2f}'
...
def intelligent_get_nifty_option(signal_type, target_premium=100):
...
def position_open(signal_type, net_mtm):
    ...
    try:
        ...
        instrument = intelligent_get_nifty_option(signal_type, nifty_option_premium)
                
        if not instrument or abs(instrument['ltp'] - nifty_option_premium) > nifty_option_premium_mask:
            update_terminal(f"Position: Skip {instrument['tradingsymbol']}|{instrument['ltp']}")
            return  # Skip if no suitable option found
        
        logging.info(f"\nPosition: Buying: {signal_type} - {instrument['tradingsymbol']} @ {instrument['ltp']}...")
        if not global_config['system']['virtual_env']:
            ...
            logging.info(f"Position: Buy - Ok: Order ID: {order_id}")
        else:
            logging.info('Virtual: Buy - Ok: Order placed')
        
        global_websocket_manager.subscribe([instrument['instrument_token']])
        global_current_position = {
            'type': signal_type,
            'instrument': instrument,
            'entry_price': instrument['ltp'],
            'sl': instrument['ltp'] - (global_config['rules']['risk']['stop_loss']),
            'sl_25': 0,
            'target': instrument['ltp'] + (global_config['rules']['risk']['target']),
            'tsl': 0,
            'tsl_25': 0,
            'tsl_50': 0,
            'tsl_75': 0,
        ...
        # send telegram notification
        message = f"{CONST_SYMBOL_DICT['Buy']} {CONST_SYMBOL_DICT[signal_type]} Order Buy {instrument['tradingsymbol']} {lot_size}Qty @ {instrument['ltp']}"
...
def manage_position(tick, net_mtm, position_mtm):
    ...
    # Price < SL
    current_price = tick['last_price']
    if current_price <= (global_current_position['sl'] - sl_buffer):
        if (global_config['system']['virtual_env']):
    ...
    # Price <= SL_25
        ...
        logging.info(f"\nSL-Hit: SL_25")
    # Price >= 25% target
    elif ((global_current_position['sl_25'] == 0) and (current_price >= (global_current_position['entry_price'] + (0.25 * target_points)))):
        global_current_position['sl_25'] = global_current_position['sl'] + (0.25 * target_points)
        logging.info(f"\nRisk: SL-Moved: 25% Target-Hit: SL_25 - {global_current_position['sl_25']}")
    # Price >= 50% target
    elif ((global_current_position['besl_50'] == 0) and (current_price >= (global_current_position['entry_price'] + 0.5 * (global_current_position['target'] - global_current_position['entry_price']) - (trailing_buffer)))):
        global_current_position['besl_50'] = global_current_position['entry_price'] + global_config['rules']['risk']['besl_trailing']
        logging.info(f"\nRisk: BreakEven-Lock: 50% Target-Hit: BESL_50 - {global_current_position['besl_50']}")
    # Price >= 75% target
    elif ((global_current_position['besl_75'] == 0) and (current_price >= (global_current_position['entry_price'] + (0.75 * target_points) - trailing_buffer))):
        global_current_position['besl_75'] = global_current_position['entry_price'] + (0.5 * target_points)
        logging.info(f"\nRisk: BreakEven-Lock: 75% Target-Hit: BESL_75 - {global_current_position['besl_75']}")
    # Price >= Target
        ...
        logging.info(f"\nRisk: Target-Hit-Lock: TSL - {global_current_position['tsl']} and Target - {global_current_position['target']}")        
    # Price >= TSL & Price >= 25% of new Target
        ...
        logging.info(f"\nRisk: 25% Trailing-Hit: TSL_25 - {global_current_position['tsl_25']}")
    # Price >= TSL & Price >= 50% of new Target
        ...
        logging.info(f"\nRisk: 50% Trailing-Hit: TSL_50 - {global_current_position['tsl_50']}")
    # Price >= TSL & Price >= 75% of new Target
        ...
        logging.info(f"\nRisk: 75% Trailing-Hit: TSL_75 - {global_current_position['tsl_75']}")
    # Price <= TSL_75
...
def position_close(tick, current_price=0, position_running_mtm=None, net_mtm=None):
    ...
    try:
        ...
        if not global_config['system']['virtual_env']:
            ...
            logging.info(f'Virtual: Sell - Ok: Order placed')

        # Unsubscribe from tick as no longer needed
        global_websocket_manager.unsubscribe([tick['instrument_token']])

        # send telegram notification
        message = f"{CONST_SYMBOL_DICT['Sell']} {CONST_SYMBOL_DICT[global_last_exit_reason]} Order Sell {global_current_position['instrument']['tradingsymbol']} @ {current_price} - {global_last_exit_reason}"
        if position_running_mtm is not None:           
            pnl_symbol = (
                f"{CONST_SYMBOL_DICT['green']}{CONST_SYMBOL_DICT['Up-Triangle']}"
                if position_running_mtm >= 0
                else f"{CONST_SYMBOL_DICT['red']}{CONST_SYMBOL_DICT['Down-Triangle']}"
            )
            message += f" | {pnl_symbol}{CONST_SYMBOL_DICT['INR']}{position_running_mtm:.2f}"            
            pnl_symbol = (
                f"{CONST_SYMBOL_DICT['green']}{CONST_SYMBOL_DICT['Up-Triangle']}"
                if (net_mtm+position_running_mtm) >= 0
                else f"{CONST_SYMBOL_DICT['red']}{CONST_SYMBOL_DICT['Down-Triangle']}"
            )
            message += f" | {CONST_SYMBOL_DICT['Net-m2m']}{pnl_symbol}{CONST_SYMBOL_DICT['INR']}{net_mtm+position_running_mtm:.2f}"
        ...
        logging.error(f"Position: Selling Error: {e}")
...
class ColoredLogger:
    class ColoredFormatter(logging.Formatter):
        ...
        def format(self, record):
            ...
            red_terms = ["False", "PE", "Sell", "low", "SL-Hit", "sl", "Loss",
...
def main():
    ...
    try:
        ...
        message = f"{CONST_SYMBOL_DICT['System-Start']} System: Init Buying: {global_config['name']} - {global_config['version']}..."
        logger.info(message)
